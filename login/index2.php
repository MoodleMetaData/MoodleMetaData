...
$testsession = optional_param('testsession', 0, PARAM_INT); // Rest session works properly.
...
$PAGE->set_url("$CFG->httpswwwroot/login/index2.php");
...
// The $authsequence = get_enabled_auth_plugins(true); // auths, in sequence.
$authsequence = array('manual'); // Only allow manual logins.
...
if ($user !== false or $frm !== false or $errormsg !== '') {
    ...
    // This page is cloned off of the index.php file. I'm not fixing the logic to satisfy the code checker.
    $site2 = $site;
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies.
    ...
    if ($user) {
        ...
        redirect(new moodle_url($CFG->httpswwwroot . '/login/index2.php', array('testsession' => $USER->id)));
...
if ($sessionhastimedout and !data_submitted()) {
...
if (empty($SESSION->wantsurl)) {
    $SESSION->wantsurl = (array_key_exists('HTTP_REFERER', $_SERVER) &&
        ...
        $_SERVER["HTTP_REFERER"] != $CFG->httpswwwroot . '/login/index2.php' &&
        $_SERVER["HTTP_REFERER"] != $CFG->httpswwwroot . '/login/guest.php'
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new moodle_url('/login/index2.php'));
}
$PAGE->set_title("$site->fullname: $loginsite");
$PAGE->set_heading("$site->fullname");
...
if (isloggedin() and !isguestuser()) {
    ...
    $logout = new single_button(new moodle_url($CFG->httpswwwroot . '/login/logout.php', array('sesskey' => sesskey(),
        'loginpage' => 1)), get_string('logout'), 'post');
    $continue = new single_button(new moodle_url($CFG->httpswwwroot . '/login/index2.php', array('cancel' => 1)),
