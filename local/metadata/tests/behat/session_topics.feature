...
Feature: Session tab
  ...
  Scenario: Adding and deleting topics for an existing session, with saved and unsaved topic deleted topics
    ...
    Then the "all_topics[0][]" select box should contain "Saved Deleted Topic"
    Then the "all_topics[0][]" select box should contain "Unsaved Deleted Topic"
    When I set the field "all_topics[0][]" to "Saved Deleted Topic, Unsaved Deleted Topic"
    And I press "delete_topics[0]"
    Then the "all_topics[0][]" select box should contain "Undeleted"
    Then the "all_topics[0][]" select box should not contain "Saved Deleted Topic"
    Then the "all_topics[0][]" select box should not contain "Unsaved Deleted Topic"
    And I press "Save changes"
    Then the "all_topics[0][]" select box should contain "Undeleted"
    Then the "all_topics[0][]" select box should not contain "Saved Deleted Topic"
    Then the "all_topics[0][]" select box should not contain "Unsaved Deleted Topic"
    Then the "all_topics[0][]" select box should not contain "Saved Deleted Topic"
    Then the field "sessiontitle[0]" matches value "Title"
