...
$sql = 'select gi.id,list.fullname,list.idnumber,list.courseid from (select c.idnumber,c.fullname,courseid,parent,count(courseid) as amount from mdl_grade_categories g LEFT JOIN mdl_course c ON (g.courseid = c.id) where parent IS NULL group by c.idnumber,c.fullname,courseid,parent having count(courseid) > 1) as list JOIN mdl_grade_categories gi ON (list.courseid = gi.courseid) where gi.parent IS NULL order by list.courseid,gi.id';
...
//    exit;
//}
echo 'Courses With Duplicate Grade Categories:<br>';
