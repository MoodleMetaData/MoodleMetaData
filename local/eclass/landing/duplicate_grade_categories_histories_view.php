...
$records = $DB->get_recordset_sql($sql, array($_GET['courseid']));
...
if (isset($_POST['ids'])) {
    ...
    elseif (!$DB->delete_records_select("grade_categories", $where)) {
...
//    exit;
//}
echo "Grade Categories Histories Of Course: {$_GET['courseid']}<br>";
...
echo '<th>Course id</th><th>Category Full Name</th><th>Old ID</th><th>Depth</th><th>Path</th><th>Aggregation</th><th>Choose Category id to keep</th>';
foreach ($processed_rec as $record) {
    echo '<tr><td><a href="' . $CFG->wwwroot . "/course/view.php?id=" . $record->courseid . "\" target='_blank'>$record->courseid";
    if (!empty($record->deletable)) {
        echo " (fixed)";
    }
    echo '</a></td><td>' . $record->fullname . '</td>';
    echo "<td>$record->oldid</td>";
    echo "<td>$record->depth</td>";
    echo "<td>$record->path</td>";
    echo "<td>$record->aggregation</td>";
    echo '<td><label for="' . $record->courseid . '_blank">All</label><input type="radio" id="' . $record->courseid . '_blank" name="ids[' . $record->courseid . ']" value="' . $record->courseid . '_' . 'blank"/><br/>';
    if (empty($record->deletable)) {
...
function process_records(&$processed_rec, $records) {
    foreach ($records as $record) {
        if (empty($processed_rec[$record->courseid])) {
            ...
            $curr->oldid = $record->oldid;
            $curr->source = $record->source;
            $curr->parent = $record->parent;
            $curr->depth = $record->depth;
            $curr->path = $record->path;
            $curr->aggregation = $record->aggregation;
