...
function confirm_save() {
    return do_confirm("Are you sure you want to save record?");
}
...
echo '</head><body>';
echo "<p><a href='course_grade_categories_view.php?courseid={$_GET['courseid']}'>Go To Grade Categories View</a></p>";
...
if (isset($_POST['id']) && isset($_POST['action'])) {
    switch($_POST['action']){
        case 'delete':
            ...
            if (!$DB->delete_records_select("grade_items", $where)) {
...
$sql = 'select ' . implode(',',$a_fields) . ' from {grade_items} gi where courseid='. $_REQUEST['courseid'];
...
//    var_dump($a_records);
    ...
    foreach($a_records as $a_rec){
        $body .= '<tr>';
        $body .= "<td><a href='course_grade_item_edit.php?id={$a_rec->id}'>edit</a><br/><input name='id' value='". $a_rec->id ."' type='radio' />". $a_rec->id ."</td>";
    ...
    $form = <<<FORM
<form method='post' action='course_grade_item_view.php?courseid=${_REQUEST['courseid']}'>
    <table>
    $headers_row
    $body
    </table>
