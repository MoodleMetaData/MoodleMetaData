...
function get_context_nav(navigation_node $contextnode, $context) {
    ...
    if (has_capability('local/contextadmin:renameroles', $context)) {
        ...
        if (!$contextnode->get('roleedit')) {
            $contextnode->add(get_string('rolerenaming'), $url, navigation_node::TYPE_SETTING, null, 'roleedit',
                new pix_icon('i/settings', ''));
        }
    }

    if (has_capability('local/contextadmin:changevisibilty', $context) or
        has_capability('local/contextadmin:editowncatsettings', $context)) {
