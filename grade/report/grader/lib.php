...
class grade_report_grader extends grade_report {
            ...
            foreach ($students as $userid => $items) {
                ...
                foreach ($items as $itemid => $postedvalue) {
                    ...
                    if ($datatype === 'grade') {
                        ...
                        // eClass modification CTL-742
                        // If the old value is actually newer then the posted value we should ignore the posted value
                        // Updated in CTL-868 as it was breaking a test.
                        if (isset($data->timepageload) && $oldvalue->timemodified > $data->timepageload) {
                            if ($gradeitem = $oldvalue->grade_item) {
                                $user = $DB->get_record('user', array('id' => $userid), 'id, firstname, lastname');
                                $gradestr = new stdClass();
                                $gradestr->username = fullname($user);
                                $gradestr->itemname = $gradeitem->itemname;
                                $gradestr->postedvalue = $postedvalue;
                                $warnings[] = get_string('updatedafterload', 'grades', $gradestr);
                            }
                            continue;
                        }
    ...
    public function load_users() {
		...
		Extra Comments:
		 ************/
		$showempid = $this->get_pref('showempid');
		if($showempid) {
			if (has_capability('gradereport/grader:viewempid', $this->context)) {
