...
M.block_library._isEmpty = function(e) {//_isEmpty(e){
    return (e===null||typeof e==="undefined"||e===undefined);
}
M.block_library.ebscoHostSearchParse = function(ebscohostsearchtext, ebscohostsearchmode) { //ebscoHostSearchParse(ebscohostsearchtext,ebscohostsearchmode) {
...
M.block_library.ebscoHostSearchGo = function(form) {//function ebscoHostSearchGo(form) {
    ...
    var ebscohostsearchtext = M.block_library.chkObject(form.ebscohostsearchtext,'');
    var ebscohostkeywords = M.block_library.chkObject(form.ebscohostkeywords,'');
    var ebscohostsearchsrc = M.block_library.chkObject(form.ebscohostsearchsrc,'');
    var ebscohostsearchmode = M.block_library.chkObject(form.ebscohostsearchmode,'\+AND\+');
    var ebscohostwindow = parseInt(M.block_library.chkObject(form.ebscohostwindow,0));
    var ebscohosturl = M.block_library.chkObject(form.ebscohosturl,'http://search.ebscohost.com/login.aspx?');
    ...
    if (ebscohosturl.indexOf('eds-live') == -1 && ebscohosturl.indexOf('edspub-live') === -1) {
        if (ebscohostsearchsrc == "db" || ebscohostsearchsrc == "dbgroup") {
            var ebscohostdatabases = M.block_library.getSelectedDatabases(form.cbs,ebscohostsearchsrc);
    ...
    if (!M.block_library._isEmpty(cbFT) && cbFT.checked) {
    ...
    if (!M.block_library._isEmpty(cbPR) && cbPR.checked) {
    ...
    if (!M.block_library._isEmpty(cbCO) && cbCO.checked) {
    ...
    if (!M.block_library._isEmpty(cbLC) && cbLC.checked) {
    ...
    if (ebscohostkeywords!="")
        ebscohostkeywords=M.block_library.ebscoHostSearchParse(ebscohostkeywords,ebscohostsearchmode)+'\+AND\+';
    ...
    if(!M.block_library._isEmpty(authorSelector) && authorSelector.checked) {
        ebscohosturl+='&bquery=AU+('+ebscohostkeywords+M.block_library.ebscoHostSearchParse(ebscohostsearchtext,ebscohostsearchmode)+')';
    } else if(!M.block_library._isEmpty(titleSelector) && titleSelector.checked) {
        ebscohosturl+='&bquery=TI+('+ebscohostkeywords+M.block_library.ebscoHostSearchParse(ebscohostsearchtext,ebscohostsearchmode)+')';
    } else {
        ebscohosturl+='&bquery='+ebscohostkeywords+M.block_library.ebscoHostSearchParse(ebscohostsearchtext,ebscohostsearchmode);
    }

    if (ebscohostwindow)
        window.open(ebscohosturl, 'EBSCOhost');
    else
        window.open(ebscohosturl, '_blank');
...
M.block_library.getSelectedDatabases = function(ob, src) {//function getSelectedDatabases(ob,src) {
    var databases = M.block_library.getMultiple(ob);
    if(databases == "")
        return -1;
...
M.block_library.getMultiple = function(ob) { //function getMultiple(ob) {
...
M.block_library.highlight = function(id, currentClass, checkBoxId) { //function highlight(id, currentClass, checkboxId) {
    var checkbox_element = M.block_library.findObj(checkboxId);
    var row_element = M.block_library.findObj(id);

    row_element.className = checkbox_element.checked ? 'selected' : currentClass;
    if(checkbox_element.checked==false)
        M.block_library.findObj('cball').checked=false;
}

M.block_library.SelectAllCheckBoxes = function(e) { //function SelectAllCheckBoxes(e) {
    ...
    for(var i=0;i<cbs.length;i++){
        ...
        if (row.id) {
            M.block_library.highlight(row.id, (i % 2) ? 'two' : 'one', checkbox.id);
        }
    }
}

M.block_library.findObj = function(n, d) {//function findObj(n, d) {
    var p,i,x;
    ...
    for(i=0;!x&&d.layers&&i<d.layers.length;i++)
        x=M.block_library.findObj(n,d.layers[i].document);
...
M.block_library.chkObject = function(f, d) { //function chkObject(f,d) {
    if (f != null)
        return f.value;
    else
        return d;
