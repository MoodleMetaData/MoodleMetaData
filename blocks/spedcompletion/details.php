...
/**
 * Block for displaying logged in user's sped course completion status
 *
 * @package    block
 * @subpackage sped completion
...
// Print header.
$page = get_string('completionprogressdetails', 'block_spedcompletion');
$title = format_string($course->fullname) . ': ' . $page;

$PAGE->navbar->add($page);
$PAGE->set_pagelayout('report');
$PAGE->set_url('/blocks/spedcompletion/details.php', array('course' => $course->id, 'user' => $user->id));
...
if ($coursecomplete) {
    echo get_string('spedcomplete', 'block_spedcompletion');
...
if (empty($completions)) {
    ...
    echo html_writer::tag('th', get_string('criteriagroup', 'block_spedcompletion'), array('class' => 'c0 header',
        'scope' => 'col'));
    echo html_writer::tag('th', get_string('criteria', 'completion'), array('class' => 'c1 header', 'scope' => 'col'));
    echo html_writer::tag('th', get_string('requirement', 'block_spedcompletion'), array('class' => 'c2 header', 'scope' => 'col'));
...
echo $OUTPUT->single_button($courseurl, get_string('returntocourse', 'block_spedcompletion'), 'get');
