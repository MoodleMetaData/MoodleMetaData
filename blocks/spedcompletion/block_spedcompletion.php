 ...
 */
class block_spedcompletion extends block_base
{
    ...
    public function init() {
        $this->title = get_string('pluginname', 'block_spedcompletion');
    }
    ...
    public function get_content() {
        ...
        if ($info->is_tracked_user($USER->id)) {
            ...
            if (!empty($activities)) {
                $a = new stdClass();
                $a->first = $activitiescomplete;
                $a->second = count($activities);

                $row = new html_table_row();
                $row->cells[0] = new html_table_cell(get_string('activitiescompleted', 'completion'));
                $row->cells[1] = new html_table_cell(get_string('firstofsecond', 'block_spedcompletion', $a));
                $row->cells[1]->style = 'text-align: right;';
                $srows[] = $row;
            }
            ...
            if ($pendingupdate) {
                ...
                $content .= get_string('spedcomplete', 'block_spedcompletion');
            ...
            $details = new moodle_url('/blocks/spedcompletion/details.php', array('course' => $course->id));
