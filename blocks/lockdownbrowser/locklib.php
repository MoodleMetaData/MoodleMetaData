...
require_once("$CFG->dirroot/blocks/lockdownbrowser/locklibcfg.php");
...
function lockdownbrowser_tokens_free()
{
    ...
    // Catch an error thrown during install
    // No impact of returning zero. The method is only used on the admin page to list free tokens.
    try {
...
function lockdownbrowser_allocate_token1($sesskey, $url)
{
    ...
    $msg = '<script> document.location = \'' . $url . '&' .  $CFG->block_lockdownbrowser_LDB_TOKEN1_COOKIE . '=' . $obj1->token1 . '\'</script>' ;
    echo $msg ;
    return 0;
}
function lockdownbrowser_validate_token2()
{
    global $CFG, $DB;
    $valid = false;
    if (isset($_COOKIE[$CFG->block_lockdownbrowser_LDB_TOKEN2_COOKIE]) && isset($_COOKIE[$CFG->block_lockdownbrowser_LDB_SESSION_COOKIE.$CFG->sessioncookie])) {
...
function lockdownbrowser_generate_tokens_student()
{
    ...
    $mkt = false;
    $rf = $DB->count_records('block_lockdownbrowser_toke') - $DB->count_records('block_lockdownbrowser_sess');
    if (($rf < 10) || ($rf < 100 && ($rf % 5 == 0)) || ($rf < 250 && ($rf % 20 == 0))) {
        $mkt = true;
    }
    if ($mkt) {
         return lockdownbrowser_generate_tokens();
...
function lockdownbrowser_generate_tokens_debug()
{
    ...
    $ok = true;
    $timenow = time();
    $plain = $timenow . $CFG->block_lockdownbrowser_LDB_SERVERID . $CFG->block_lockdownbrowser_LDB_TSERVER_AKEY . $CFG->block_lockdownbrowser_LDB_SERVERSECRET;
    $auth = md5($plain);
    $f1 = sprintf($CFG->block_lockdownbrowser_LDB_TSERVER_FORM1, $CFG->block_lockdownbrowser_LDB_SERVERID );
    $f2 = sprintf($CFG->block_lockdownbrowser_LDB_TSERVER_FORM2, $auth, $CFG->block_lockdownbrowser_LDB_SERVERNAME, $CFG->block_lockdownbrowser_LDB_SERVERTYPE);
    $formdata = $f1 . $timenow . $f2;
    $url = $CFG->block_lockdownbrowser_LDB_TSERVER_1 . $CFG->block_lockdownbrowser_LDB_TSERVER_ENDPOINT;
    ...
    for ($ix = 0; $ix < $CFG->block_lockdownbrowser_LDB_TSERVER_SET; $ix++) {
        ...
        $tc->token2 = substr($decoderesp, $rpos + $CFG->block_lockdownbrowser_LDB_TSERVER_T2P, $CFG->block_lockdownbrowser_LDB_TSERVER_T2L);
        $tc->sesskey = 0;
...
function lockdownbrowser_generate_tokens()
{
    ...
    $resp_b64 = base64_decode($resp);
    $decoderesp = mcrypt_decrypt(MCRYPT_BLOWFISH , $fk2, $resp_b64, MCRYPT_MODE_ECB, "01234567");
    if (strlen($decoderesp) != ($CFG->block_lockdownbrowser_LDB_TSERVER_SET * $CFG->block_lockdownbrowser_LDB_TSERVER_REC)) {
...
function lockdownbrowser_check_for_lock() {
	...
	else if ( "review.php" == $script && ( $script."NONE".$discriminator == $_SESSION['LOCKDOWNBROWSER_CONTEXT'] || $script."VALID".$discriminator == $_SESSION['LOCKDOWNBROWSER_CONTEXT'] ) ) {
		return 1;
	}
		  	...
		  	if ($ok) {
				...
				//}
                                if (!empty($CFG->block_lockdownbrowser_LDB_DOWNLOAD)) {
                                    $myerror .= "<div style='font-size: 125%; color:black; text-align: center;'>".
                                                get_string('click','block_lockdownbrowser')." <a href='".$CFG->block_lockdownbrowser_LDB_DOWNLOAD."' target='_blank'>".get_string('here','block_lockdownbrowser')."</a>".get_string('todownload','block_lockdownbrowser').
                                                "</div>";
                                } else {
                                    $myerror .= "<div style='font-size: 125%; color:black; text-align: center;'>".
