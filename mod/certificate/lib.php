...
function certificate_update_instance($certificate) {
    ...
    // The original code assumes that id might be in $certificate->instance
    // for some reason. Either way, if this test is added in case
    // $certificate->id is the only one provided.
    if (!empty($certificate->instance)) {
...
function certificate_reset_userdata($data) {
    ...
    if (!empty($data->reset_certificate)) {
        ...
        $status[] = array('component' => $componentstr, 'item' => get_string('removecert', 'certificate'),
            'error' => false);
    }
