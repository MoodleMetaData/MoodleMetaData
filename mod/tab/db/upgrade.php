...
function xmldb_tab_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2012101500) {
        ...
        // Define field fileattachment to be added to tab_content
        $table = new xmldb_table('tab_content');
        $field = new xmldb_field('fileattachment', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'timemodified');

        // Conditionally launch add field fileattachment
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // tab savepoint reached
        upgrade_mod_savepoint(true, 2012101500, 'tab');
    }
