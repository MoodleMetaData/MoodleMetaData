...
function xmldb_respondusws_install() {
    respondusws_install_testing_filter();
}
...
/**
 * Call this instead of respondusws_install() to avoid calling respondusws_install() during testing, avoiding
 * PHPUnit fails.
 *
 * @throws moodle_exception
 */
function respondusws_install_testing_filter() {
    /*
     * PHPUNIT_UTIL is a constant that exist only during running phpunit_behat.setup.sh. If that is defined, don't run
     * respondusws_install(), since that will add unnecessary course_modules and context rows.
     */
