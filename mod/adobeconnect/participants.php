...
if ($roleid) {
    ...
    if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
        ...
        if (!empty($userstoassign)) {
            ...
            add_to_log($course->id, 'role', 'assign', 'mod/adobeconnect/participant.php?contextid='.$context->id.
    ...
    // Process incoming role removals.
    if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
        $userstounassign = $currentuserselector->get_selected_users();
        if (!empty($userstounassign)) {

            foreach ($userstounassign as $removeuser) {
                // Remove only roles that are added manually, no messing with other components!
            ...
            $rolename = $assignableroles[$roleid];
            add_to_log($course->id, 'role', 'unassign', 'mod/adobeconnect/participant.php?contextid='.$context->id.
                '&roleid='.$roleid, $rolename, '', $USER->id);
            // Counts have changed, so reload.
            list($assignableroles, $assigncounts, $nameswithcounts) =
                adobeconnect_get_assignable_roles($context, ROLENAME_BOTH, true);
...
<?php
    ...
    foreach ($assignableroles as $roleid => $notused) {
        ...
        if (0 < $assigncounts[$roleid] && $assigncounts[$roleid] <= MAX_USERS_TO_LIST_PER_ROLE) {
            $namefields = get_all_user_name_fields(false, 'u', 'u.');
            $roleusers = get_role_users($roleid, $context, false, 'u.id,' . implode(",", $namefields));
            if (!empty($roleusers)) {
                $strroleusers = array();
    ...
    $table->attributes = array('style' => $style);
