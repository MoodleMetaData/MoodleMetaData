...
class format_collblct_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        $this->togglelib = new collblct_togglelib;
        $this->courseformat = course_get_format($page->course); // Needed for collapsed topics settings retrieval.

        /* Since format_collblct_renderer::section_edit_controls() only displays the 'Set current section' control when editing
    ...
    public function print_single_section_page($course, $sections, $mods, $modnames, $modnamesused, $displaysection) {
        /*-------------------------------------------------------------------------->
         * Note: this is a copy of print_single_section_page from course/format/renderer.php
         * I've had to add this back in in order to add the hooks properly.
         *<------------------------------------------------------------------------*/
        global $PAGE;
    ...
    protected function toggle_all() {
        ...
        $o .= html_writer::tag('a', get_string('collblctopened', 'format_collblct'),
                               array('class' => 'on '.$this->tctoggleiconsize, 'href' => '#', 'id' => 'toggles-all-opened'));
        $o .= html_writer::tag('a', get_string('collblctclosed', 'format_collblct'),
    ...
    protected function display_instructions() {
        $o = html_writer::start_tag('li', array('class' => 'tcsection main clearfix', 'id' => 'collblct-display-instructions'));
        ...
        $o .= html_writer::tag('p', get_string('instructions', 'format_collblct'), array('class' => 'collblct-display-instructions'));
        $o .= html_writer::end_tag('div');
        $o .= html_writer::end_tag('div');
        $o .= html_writer::end_tag('li');
