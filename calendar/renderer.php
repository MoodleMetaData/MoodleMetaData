    ...
    public function show_day(calendar_information $calendar, moodle_url $returnurl = null) {
        ...
        if (empty($events)) {
            ...
            foreach ($events as $event) {
                $event->description = str_replace('<img ', '<img class="img-responsive" ',$event->description);
            ...
            if (isset($durationbyday[$day])) {
                ...
                foreach($durationbyday[$day] as $eventindex) {
                    ...
                    if (!empty($events[$eventindex]->class)) {
                        ...
                        /*
                         * This is unlikely to happen since moodle always requires a "Type of event" when entering a new event.
                         * Just to be defensive, this is place here instead.
                         */
                        $attributes['class'] = 'events-underway';
                    }
                    $cell->text .= html_writer::tag('li', '['.format_string($events[$eventindex]->name,true).']', $attributes);
                }
                $cell->text .= html_writer::end_tag('ul');
            }
            $row->cells[] = $cell;
